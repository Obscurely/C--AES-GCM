    ...
    static class AES
    {
        ...
        public static string Encrypt(string text, string base64key, string authenticationTag = null)
        {
            ...
            using (AesGcm cipher = new(key))
            {
                ...
                {
                    authenticationTag = Convert.ToBase64String(Encoding.UTF8.GetBytes(authenticationTag));
                    cipher.Encrypt(nonce, Encoding.UTF8.GetBytes(text), cipherText, tag, Encoding.UTF8.GetBytes(authenticationTag));
                }
        ...
        public static string Decrypt(string encryptedText, string base64key, string authenticationTag = null)
        {
            ...
            byte[] cipherText = SubArray(encryptedTextBytes, TagSize + NonceSize, encryptedTextBytes.Length - tag.Length - nonce.Length);
            ...
            using (AesGcm cipher = new(key))
            {
                ...
                {
                    authenticationTag = Convert.ToBase64String(Encoding.UTF8.GetBytes(authenticationTag));
                    cipher.Decrypt(nonce, cipherText, tag, decryptedText, Encoding.UTF8.GetBytes(authenticationTag));
                }
            }
